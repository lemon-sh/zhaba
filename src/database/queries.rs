...
pub static DELETE_POST: &str = "delete from posts where id = ?";
pub static SELECT_POSTS_BOARD: &str =
    "select id,content,image,ip,asn,mnt,time from posts where board = ? order by time desc limit ?";
